...
function [test_prediction] = knn_classifier_pca(KNN, tr_images, ...
    tr_labels, test_images, pca, pcamean)
...
% Alter number of components
tr_pca_mean = repmat(pcamean, ntr, 1);
test_pca_mean = repmat(pcamean, ntest, 1);
tr_images_trans = (tr_images' - tr_pca_mean) * pca;
test_images_trans = (test_images' - test_pca_mean) * pca;
tr_images = tr_images_trans';
test_images = test_images_trans';

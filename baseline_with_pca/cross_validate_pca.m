...
function mean_acc = cross_validate(K, tr_images, tr_labels, nfold, ...
    tr_identity, pca, pcamean, numberComponents)

pca = pca(:,1:numberComponents);
...
for i=1:nfold
  ...
  predi = knn_classifier_pca(K, tr_images(:, :, traini_ids), ...
      tr_labels(traini_ids), tr_images(:, :, testi_ids), pca, pcamean);
